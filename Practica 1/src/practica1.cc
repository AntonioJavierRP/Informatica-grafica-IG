...
Cubo cube(10);
Tetraedro tetrahedron(10);
char tecla;
bool modo = false;	// Por defecto en modo tetraedro.
...
void draw_objects()
{
	...
	switch(tecla){
		...
		case 'C':		// Ajedrez
			if(modo)
				cube.draw_solid_chess();
			else
				tetrahedron.draw_solid_chess();
			break;
		default:
...
switch (Tecla1){
	case GLUT_KEY_F1: modo = false;break;		// Tetraedro
	case GLUT_KEY_F2: modo = true;break;		// Cubo
